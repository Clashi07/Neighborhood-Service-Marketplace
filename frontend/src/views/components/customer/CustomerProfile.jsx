...
import { getUserProfile, updateUserProfile, changePassword, deactivateAccount, updateNotificationPreferences } from '../../../services/userService';
...
const CustomerProfile = () => {
  ...
  const handleDeactivateAccount = async () => {
    if (!window.confirm('Are you sure you want to deactivate your account? This will temporarily disable your account.')) {
      return;
    }
    setSaving(true);
    try {
      await deactivateAccount();
      setSuccess('Account deactivated successfully. You will be logged out now.');
      setTimeout(async () => {
        await logout();
        navigate('/login');
      }, 2000);
  ...
  const handleNotificationChange = async (type, value) => {
    setSaving(true);
    try {
      const updatedPrefs = { ...notificationPrefs, [type]: value };
                ...
                <Card.Body className="p-4">
                  ...
                  <Card className="border border-danger">
                    <Card.Body className="d-flex justify-content-between align-items-center">
                      ...
                      <Button variant="outline-danger" size="sm" onClick={handleDeactivateAccount} disabled={saving}>
                        {saving ? <><Spinner size="sm" className="me-2" />Processing...</> : 'Deactivate'}
                      </Button>
                    </Card.Body>
                  </Card>
                </Card.Body>
